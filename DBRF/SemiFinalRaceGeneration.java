...
public class SemiFinalRaceGeneration {
	...
	//							System.out.println(tmCat.get(0).get(0).getTeamName());
		...
		//if the semi finals have already been generated once, just add all the UI compnents to the panel instead
		else if(FestivalObject.generatedSemiFinalRacesEh == true) {
			...
			int temp2 = temp + (int)Math.ceil((double)FestivalObject.teamsArray.size() / (double)FestivalObject.numOfLanes);	//get the race number that is the end of the semi finals
			
			rowCounter = 0;		//reset the counter for row placement
			
			//loop from the start of the semi finals until the end of them
			for(int i = temp; i < temp2; i++) {
				...
				for(int k = 0; k < FestivalObject.racesArray.get(i).getTeamsRacing().size(); k++) {
					...
					panel.add(FestivalObject.racesArray.get(i).getTeamsRacing().get(k).getPlaceLabel(2), "flowx,cell 0 " + rowCounter + ",growx,aligny center");	//add the place label
					...
					panel.add(FestivalObject.racesArray.get(i).getTeamsRacing().get(k).getTimeFlag(2), "cell 4 " + rowCounter + ",aligny center");		//add the time change flag
					
					panel.add(FestivalObject.racesArray.get(i).getTeamsRacing().get(k).getTimeInputField(2), "cell 5 " + rowCounter + ",growx,aligny center");	//add the time input field
					
					panel.add(FestivalObject.racesArray.get(i).getTeamsRacing().get(k).getLockButton(2), "cell 6 " + rowCounter);		//add the lock button
